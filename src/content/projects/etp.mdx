...
A production system across three surfaces, built solo:
...
All three consume a shared API client — TypeScript, generated from the
OpenAPI spec and published to GitHub Packages — rather than each hand-writing
its own fetch layer. That client is infrastructure the surfaces share, not a
fourth surface in its own right.
